var MAIN = {
  ...
  // Menu desktop hover item
  menuDesktop: function () {
    const all_danh_muc_menu = document.querySelectorAll(
      ".menu-desktop-danh-muc ul li"
    )
    const all_menu_item_danh_muc = document.querySelectorAll(
      ".menu-item-danh-muc"
    )
    const all_menu_item_sp = document.querySelectorAll(".menu-item-sp")

    // Hàm kích hoạt phần tử ban đầu
    function activateInitialItems() {
      all_danh_muc_menu[2].querySelector("a").classList.add("text-active")
      all_danh_muc_menu[2]
        .querySelector("a i.fa-chevron-right")
        .classList.remove("translate-x-20")
      all_menu_item_danh_muc.forEach((i) => {
        i.getAttribute("category") == 3
          ? i.classList.remove("hidden")
          : undefined,
          i.querySelector("li a").getAttribute("category-item") == 3.1
            ? i.querySelector("li a").classList.add("active-bg-menu")
            : undefined
      })
      all_menu_item_sp.forEach((i) => {
        i.getAttribute("category-item") == 3.1
          ? i.classList.remove("hidden")
          : undefined
      })
    }

    // Gọi hàm kích hoạt phần tử ban đầu
    activateInitialItems()

    all_danh_muc_menu.forEach((i) => {
      i.querySelector("a").addEventListener("mouseenter", () => {
        all_menu_item_sp.forEach((t) => t.classList.add("hidden"))
        all_danh_muc_menu.forEach((e) => {
          e.querySelector("a").classList.remove("text-active")
          e.querySelector("a i.fa-chevron-right").classList.add(
            "translate-x-20"
          )
        })
        i.querySelector("a").classList.add("text-active")
        i.querySelector("a i.fa-chevron-right").classList.remove(
          "translate-x-20"
        )
        all_menu_item_danh_muc.forEach((z) => {
          if (
            ...
            all_menu_item_danh_muc.forEach((d) => d.classList.add("hidden"))
      ...
      i.querySelectorAll("li a").forEach((u) => {
        u.addEventListener("mouseenter", () => {
          ...
          all_menu_item_sp.forEach((z) => {
            if (
              z.getAttribute("category-item") ===
              u.getAttribute("category-item")
            ) {
              all_menu_item_sp.forEach((d) => d.classList.add("hidden"))
              z.classList.remove("hidden")
            } else {
              z.classList.add("hidden")
            }
      ...
      all_btn_item_content.forEach((i) => {
        i.addEventListener("click", () => {
          ...
          all_content_san_pham.forEach((z) => {
            z.classList.add("hidden")
            z.classList.add("scale-y-125")
            z.classList.add("opacity-0")
            if (
              z.getAttribute("data-content") === i.getAttribute("data-content")
            ) {
              ...
              z.classList.remove("hidden")
              setTimeout(() => {
                z.classList.remove("scale-y-125")
  ...
  // Toggle menu page sp
  toggleMenuPageSp: function () {
    if (document.querySelector(".btn-toggle-menu-sp")) {
      const btn_toggle_menu_sp = document.querySelectorAll(
        ".btn-toggle-menu-sp"
      )
      const menu_page_sp_moblie = document.querySelector(
        ".menu-san-pham-moblie"
      )
  ...
  clickItemMenuSanPham: function () {
    if (document.querySelector(".menu-san-pham-moblie")) {
      const all_btn_menu_moblie = document.querySelectorAll(
        ".menu-san-pham-moblie > ul > li"
      )
      all_btn_menu_moblie.forEach((i) => {
        i.querySelector("p.btn_click_sp_moblie").addEventListener(
          "click",
          () => {
            i.querySelector("p.btn_click_sp_moblie i").classList.toggle(
              "rotate-90"
            )
            i.querySelector(".menu-item-san-pham-moblie").classList.toggle(
              "max-h-[20rem]"
            )
          }
        )
  ...
  activeIndex: function () {
    if (document.querySelector(".menu-san-pham")) {
      const allSanPham = document.querySelectorAll(
        ".menu-san-pham > ul > li > a "
      )
